...
public class BookingSystemGUI extends Application
{
	...
	public void start(Stage primaryStage)
	{
		...
		final int spacer = 50;
		Company company = new Company("samfam");
		int slot1 = 1, slot2 = 2, slot3 = 3;
		//Calendar currentCal;
		...
		// GENERIC CANCEL BUTTON FOR POPUP MENUS
		...
		ComboBox<String> bookingNameMenu = new ComboBox();
		bookingNameMenu.setPromptText("Name");
		...
		for (String item : company.getCalendar().getMonths())
		...
		ComboBox<Integer> slotBookingMenu = new ComboBox<>();
		...
		newBookingBox.getChildren().addAll(new Label("Make a Booking"), bookingNameLabel, bookingNameMenu, monthBookingLabel, monthBookingMenu, 
		...
		optionsArea.getChildren().addAll(monthMenuTitle, monthMenu, roomNumMenuTitle, roomNumMenu, loadCalButton);
	   ...
	   //***** TEMPORARY DATA ADDED TO CALENDAR FOR TESTING GUI ******//
		
		company.addPerson("Olly", "234545", "ggfgf", "fgfgf");
		company.addPerson("James", "234545", "ggfgf", "fgfgf");
		company.addPerson("Vicki", "234545", "ggfgf", "fgfgf");
		company.addPerson("Jasper", "234545", "ggfgf", "fgfgf");
		
		company.addRoom("AAAA");
		company.addRoom("BBBB");
		company.addRoom("C1C");
		company.addRoom("DDDD");
		
		company.getCalendar().addBooking(company.getRoom("AAAA"), company.getPerson("Olly"), "February", 15, 3);
		company.getCalendar().addBooking(company.getRoom("BBBB"), company.getPerson("James"), "January", 30, 1);
		company.getCalendar().addBooking(company.getRoom("C1C"), company.getPerson("Vicki"), "March", 25, 2);
		company.getCalendar().addBooking(company.getRoom("DDDD"), company.getPerson("Olly"), "December", 18, 3);
		
		for (int i = 0;i < company.getRoomsLength(); i++)
		{
			roomNumMenu.getItems().add(company.getRooms().get(i).getRoomNumber());
			bookingNameMenu.getItems().add(company.getPeople().get(i).getName());
		}
		...
		{
			company.setCurrentRoom(roomNumMenu.getValue());
		});		
		...
		monthMenu.setOnAction(e ->
		{
			...
			company.setCurrentMonth(monthMenu.getValue());
		...
		loadCalButton.setOnAction(e ->
		{
			if (!(roomNumMenu.getValue().isBlank() && monthMenu.getValue().isBlank()))
			{
				...
				List<Booking> bookingsByMonth = company.getCalendar().searchByMonth(month); //gets arraylist of bookings for selected month
				List<Booking> bookingsByMonthAndRoom = new ArrayList<>();
				for (int i = 0; i < bookingsByMonth.size(); i++)
				{
					if (bookingsByMonth.get(i).getRoomNumber().equals(room))
					{
						bookingsByMonthAndRoom.add(bookingsByMonth.get(i));
					}
				}
				...
				for (int i = 0; i < company.getCalendar().getDays().size(); i++)
				{
					Label dayLabel = new Label(company.getCalendar().getDays().get(i));
					calendarPane.add(dayLabel, i, 0);
					calendarPane.setHalignment(dayLabel, HPos.CENTER);
				}
				...
				{	
					for (int j = 0; j < 7; j++)
					{
						...
						for (int k = 0; k < bookingsByMonthAndRoom.size(); k++)
						{
							if (bookingsByMonthAndRoom.get(k).getDay() == counter + 1)
							{
								if(bookingsByMonthAndRoom.get(k).getSlot() == 1)
								{
									tempSlot1 = bookingsByMonthAndRoom.get(k).getBooking().getPersonName();
									calendarPane.add(new BookingBox(counter + 1, room, tempSlot1, bSlot2, bSlot3), j, i);
									tempSlot1 = "";
								}
								else if(bookingsByMonthAndRoom.get(k).getSlot() == 2)
								{
									tempSlot2 = bookingsByMonthAndRoom.get(k).getBooking().getPersonName();
									calendarPane.add(new BookingBox(counter + 1, room, tempSlot2, bSlot2, bSlot3), j, i);
									tempSlot1 = "";
								}
								else if(bookingsByMonthAndRoom.get(k).getSlot() == 3)
								{
									tempSlot3 = bookingsByMonthAndRoom.get(k).getBooking().getPersonName();
									calendarPane.add(new BookingBox(counter + 1, room, tempSlot3, bSlot2, bSlot3), j, i);
									tempSlot1 = "";
								}
		...
		addStudentButton.setOnAction(e -> 
		{
			...
			else if (company.addPerson(studentName.getText(), studentNumber.getText(), "", ""))
			{
				studentName.setText("");
		...
		{
			Room room = company.getCurrentRoom();
			String name = bookingNameMenu.getValue();
			String month = company.getCurrentMonth();
			int day = dayBookingMenu.getValue();
			int slot = slotBookingMenu.getValue();

			if (company.getCalendar().addBooking(room, company.getPerson(name), month, day, slot));
			{
				subMenuStage.hide();
			}
		});
